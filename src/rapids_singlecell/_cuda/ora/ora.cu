...
void register_bindings(nb::module_& m) {
        ...
        [](gpu_array_c<const float, Device> mat, int n_up, int n_bm,
           gpu_array_c<unsigned char, Device> selected, std::uintptr_t stream) {
            ...
            select_kernel<<<strided_grid(mat.shape(0), 1), 256, 0,
                ...
                (int)mat.shape(1), n_up, n_bm, selected.data());
        ...
        [](gpu_array_c<const unsigned char, Device> selected,
           ...
           gpu_array_c<int, Device> overlaps, std::uintptr_t stream) {
            if (selected.ndim() != 2 || overlaps.ndim() != 2 ||
                overlaps.shape(0) != starts.size() || offsets.size() != starts.size() ||
                overlaps.shape(1) != selected.shape(1)) {
                throw std::invalid_argument("overlap: incompatible array shapes");
            }
            if (overlaps.size() == 0) return;
            const int rows_per_block = selected.shape(1) < 32 ? WARPS_PER_BLOCK : BLOCK_SIZE;
    ...
    m.def(
        ...
        [](gpu_array_c<const unsigned char, Device> selected,
           ...
           gpu_array_c<const double, Device> p_table,
           int count, double background, gpu_array_c<double, Device> es,
           gpu_array_c<double, Device> pv, gpu_array_c<int, Device> invalid,
           std::uintptr_t stream) {
            if (selected.ndim() != 2 || es.ndim() != 2 || pv.ndim() != 2 ||
                es.shape(0) != starts.size() || es.shape(1) != selected.shape(1) ||
                pv.shape(0) != es.shape(0) || pv.shape(1) != es.shape(1) ||
                offsets.size() != starts.size() || groups.size() != starts.size() ||
                score_table.ndim() != 2 || p_table.ndim() != 2 ||
                p_table.shape(0) != score_table.shape(0) ||
                p_table.shape(1) != score_table.shape(1) || invalid.size() != 1) {
                throw std::invalid_argument("ora_lookup: incompatible array shapes");
            }
            if (es.size() == 0) return;
            const int rows_per_block = selected.shape(1) < 32 ? WARPS_PER_BLOCK : BLOCK_SIZE;
            const dim3 grid(strided_grid(selected.shape(1), rows_per_block),
            ...
            overlap_kernel<true><<<grid, BLOCK_SIZE, 0, (cudaStream_t)stream>>>(
                ...
                {groups.data(), score_table.data(), p_table.data(),
                 (int)score_table.shape(1), count, background, es.data(), pv.data(),
                 invalid.data()});
            CUDA_CHECK_LAST_ERROR(overlap_kernel);
        }, "selected"_a, nb::kw_only(), "cnct"_a, "starts"_a, "offsets"_a,
    ...
    m.def(
        ...
        [](gpu_array_c<const int, Device> a,
           ...
           std::uintptr_t stream) {
            ...
            if ((a.ndim() != 1 && a.ndim() != 2) || pv.size() != a.size() ||
                ...
                throw std::invalid_argument("fisher: incompatible array shapes");
            }
            if (a.size() == 0) return;
        ...
        }, "a"_a, nb::kw_only(), "counts"_a, "sizes"_a, "background"_a,
        "pv"_a, "alternative"_a, "gamma"_a, "tail_offset"_a, "stream"_a = 0);
}

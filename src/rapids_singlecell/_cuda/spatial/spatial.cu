...
void check_tree(gpu_array_c<const T, Device> points,
    ...
    if (points.ndim() != 2 || !points.shape(0) || !points.shape(1)
        ...
        || tree.ndim() != 2 || tree.shape(0) != points.shape(0)
        || tree.shape(1) != points.shape(1)
...
void register_tree(nb::module_& m) {
    m.def("knn", [](gpu_array_c<const T, Device> points,
                    gpu_array_c<const T, Device> tree,
        ...
        if (k < 1 || k >= n || columns.ndim() != 1 || distances.ndim() != 1
            || columns.size() != n * k || distances.size() != columns.size())
            throw std::invalid_argument("Invalid spatial kNN output buffers or k.");
        spatial_tree_search<T, false, false><<<(n + 127) / 128, 128, 0, (cudaStream_t)stream>>>(
            points.data(), tree.data(), index.data(), n, points.shape(1), k, 0,
            nullptr, nullptr, nullptr, columns.data(), distances.data());
        CUDA_CHECK_LAST_ERROR(spatial_tree_search);
    }, "points"_a.noconvert(), "tree"_a.noconvert(), "index"_a.noconvert(),
       nb::kw_only(), "k"_a, "columns"_a.noconvert(), "distances"_a.noconvert(),
       "stream"_a = 0);

    m.def("radius_count", [](gpu_array_c<const T, Device> points,
                             gpu_array_c<const T, Device> tree,
                             gpu_array_c<const long long, Device> index,
                             double radius, gpu_array_c<long long, Device> counts,
                             std::uintptr_t stream) {
        check_tree(points, tree, index, radius);
        const long long n = points.shape(0);
        if (counts.ndim() != 1 || counts.size() != n)
            throw std::invalid_argument("Invalid spatial radius count buffer.");
        spatial_tree_search<T, true, false><<<(n + 127) / 128, 128, 0, (cudaStream_t)stream>>>(
            points.data(), tree.data(), index.data(), n, points.shape(1), 0, radius,
            nullptr, counts.data(), nullptr, nullptr, nullptr);
        CUDA_CHECK_LAST_ERROR(spatial_tree_search);
    }, "points"_a.noconvert(), "tree"_a.noconvert(), "index"_a.noconvert(),
       nb::kw_only(), "radius"_a, "counts"_a.noconvert(), "stream"_a = 0);

    m.def("radius_fill", [](gpu_array_c<const T, Device> points,
                            gpu_array_c<const T, Device> tree,
        ...
        check_tree(points, tree, index, radius);
        const long long n = points.shape(0);
        if (offsets.ndim() != 1 || offsets.size() != n + 1 || rows.ndim() != 1
            || columns.ndim() != 1 || distances.ndim() != 1
            || rows.size() != columns.size() || rows.size() != distances.size())
            throw std::invalid_argument("Invalid spatial radius output buffers.");
        spatial_tree_search<T, true, true><<<(n + 127) / 128, 128, 0, (cudaStream_t)stream>>>(
            points.data(), tree.data(), index.data(), n, points.shape(1), 0, radius,
            offsets.data(), nullptr, rows.data(), columns.data(), distances.data());
        CUDA_CHECK_LAST_ERROR(spatial_tree_search);
    }, "points"_a.noconvert(), "tree"_a.noconvert(), "index"_a.noconvert(),
       nb::kw_only(), "radius"_a, "offsets"_a.noconvert(), "rows"_a.noconvert(),
       "columns"_a.noconvert(), "distances"_a.noconvert(), "stream"_a = 0);
}

template <typename T, typename R, typename C, typename Device>
void register_distances(nb::module_& m) {
    m.def("edge_distances", [](gpu_array_c<const T, Device> points,
                              gpu_array_c<const R, Device> rows,
                              gpu_array_c<const C, Device> columns,
                              gpu_array_c<T, Device> distances, std::uintptr_t stream) {
        if (points.ndim() != 2 || !points.shape(1)
            || points.shape(1) > std::numeric_limits<int>::max()
            || rows.ndim() != 1 || columns.ndim() != 1 || distances.ndim() != 1
            || (!points.shape(0) && rows.size())
            || rows.size() != columns.size() || rows.size() != distances.size())
            throw std::invalid_argument("Invalid spatial edge-distance buffers.");
        const long long n_edges = rows.size();
        if (!n_edges) return;
        spatial_edge_distances<<<(n_edges + 255) / 256, 256, 0, (cudaStream_t)stream>>>(
...
void bind_graph(nb::module_& m) {
    m.def(
        ...
        [](gpu_array_c<const R, Device> rows,
           ...
           gpu_array_c<float, Device> adj_data,
           gpu_array_c<T, Device> dst_data, std::uintptr_t stream) {
            if (rows.ndim() != 1 || columns.ndim() != 1 ||
                ...
                throw std::invalid_argument(
                    "assemble_graphs: expected 1D arrays and a nonempty indptr");
            }
            const long long n_edges = rows.shape(0);
            const long long n_obs = adj_indptr.shape(0) - 1;
            const long long nnz = n_edges + n_obs;
            if (columns.shape(0) != n_edges ||
                adj_columns.shape(0) != nnz || adj_data.shape(0) != nnz ||
                (with_distances &&
                ...
                throw std::invalid_argument(
                    "assemble_graphs: incompatible array lengths or index dtype");
            }
            ...
            const auto launch = [&](auto with_distances_tag) {
                constexpr bool WithDistances = decltype(with_distances_tag)::value;
                assemble_graphs_kernel<R, C, I, T, WithDistances>
                    <<<blocks, 256, 0, (cudaStream_t)stream>>>(
                        rows.data(), columns.data(), distances.data(),
                        n_edges, n_obs, set_diag, adj_indptr.data(),
                        dst_indptr.data(), adj_columns.data(),
                        dst_columns.data(), adj_data.data(), dst_data.data());
            };
            if (with_distances) launch(std::true_type{});
...
static void register_delaunay_bindings(nb::module_& m) {
    using Points = nb::ndarray<const double, Device, nb::shape<-1, 2>, nb::c_contig>;
    using Input = nb::ndarray<const int, Device, nb::shape<-1, 3>, nb::c_contig>;
    using Output = nb::ndarray<int, Device, nb::ndim<1>, nb::c_contig>;
    m.def(
        "validate_delaunay",
        [](Points points, Input triangles, Input opposite, double tolerance,
           Output invalid, Output seen, Output next_boundary,
           Output prev_boundary, Output boundary_count, std::uintptr_t stream) {
            const auto n = points.shape(0), nt = triangles.shape(0);
            if (n < 3 || n >= (1u << 26) || nt < n - 2 || nt > 2*n - 5 ||
                opposite.shape(0) != nt || invalid.size() != 1 ||
                boundary_count.size() != 1 || seen.size() != n ||
                next_boundary.size() != n || prev_boundary.size() != n)
                throw std::invalid_argument("Invalid Delaunay validation shapes");
            ...
            validate_triangles<<<(nt + 127) / 128, 128, 0, s>>>(
                ...
                tolerance, invalid.data(), seen.data(), next_boundary.data(),
        ...
        "opposite"_a.noconvert(), "tolerance"_a, "invalid"_a.noconvert(),
        "seen"_a.noconvert(), "next_boundary"_a.noconvert(),
        "prev_boundary"_a.noconvert(), "boundary_count"_a.noconvert(),
        "stream"_a);
    m.def(
        "fill_delaunay_edges",

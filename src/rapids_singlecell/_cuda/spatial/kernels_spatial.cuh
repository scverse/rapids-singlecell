...
template <typename T> __device__ T distance_hypot(T a, T b);
template <> __device__ float distance_hypot(float a, float b) {
    return hypotf(a, b);
}
template <> __device__ double distance_hypot(double a, double b) {
    return hypot(a, b);
}
...
__global__ void spatial_tree_search(
    const T* points, const T* tree, const long long* index,
    const long long n, const int dimensions, const int k, const double radius,
    const long long* offsets, long long* counts, long long* rows,
    ...
    long long count = 0;
    // Keep small 2D candidate lists local instead of repeatedly writing outputs.
    ...
    if (!radius_search) {
        columns += row * k;
        distances += row * k;
        if (local_topk) { columns = local_columns; distances = local_distances; }
        for (int j = 0; j < k; ++j) {
            columns[j] = -1;
    ...
    while (node >= 0) {
        ...
        const T delta = points[row * dimensions + axis]
                        - tree[node * dimensions + axis];
        ...
        if (previous == parent) {
            ...
            if (column != row) {
                T distance = fabs(points[row * dimensions]
                                  - tree[node * dimensions]);
                for (int d = 1; d < dimensions; ++d) {
                    distance = distance_hypot(
                        distance, points[row * dimensions + d]
                ...
                } else if (columns[k - 1] < 0
                           || distance < distances[k - 1]
                           || (distance == distances[k - 1]
                               && column < columns[k - 1])) {
                    int j = k - 1;
                    while (j > 0 && (columns[j - 1] < 0
            ...
            if (near < n) next = near;
            else if (far < n && fabs(delta)
                     <= (radius_search ? radius : distances[k - 1])) next = far;
        } else if (previous == near && far < n && fabs(delta)
                   <= (radius_search ? radius : distances[k - 1])) next = far;
...
__global__ void spatial_edge_distances(
    const T* coords, const R* rows, const C* columns, T* distances,
    ...
    for (int d = 1; d < dimensions; ++d)
        distance = distance_hypot(distance, coords[row + d] - coords[column + d]);

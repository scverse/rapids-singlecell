...
```{rubric} Deprecations
```
* Remove the deprecated ``use_highly_variable`` argument of {func}`~rapids_singlecell.pp.pca`, following {mod}`scanpy`. Use ``mask_var="highly_variable"`` instead of ``use_highly_variable=True`` and ``mask_var=None`` instead of ``use_highly_variable=False`` {pr}`751` {smaller}`S Dicks`

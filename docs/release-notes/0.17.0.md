...
```{rubric} Features
```
* Speed up {func}`~rapids_singlecell.pp.harmony_integrate` and make it reproducible by seeding k-means from a deterministic ``float64`` fit on a batch-stratified subsample instead of a non-reproducible ``float32`` fit over all cells. ``dtype`` now defaults to {class}`numpy.float32` {pr}`756` {smaller}`S Dicks`

...
```{rubric} Performance
```
* Rewrite the {func}`~rapids_singlecell.gr.spatial_autocorr` kernels: Moran's I and Geary's C now share one fused numerator kernel that accumulates in float64, visits only stored nonzeros for sparse input (previously every gene for every neighbour pair), reads dense input coalesced per gene, and applies permutations as an index map instead of re-materialising the permuted weights. 4–6× faster on 200k–1M cells and float32 results are now accurate to ~1e-7 instead of ~1e-4 {pr}`791` {smaller}`S Dicks`

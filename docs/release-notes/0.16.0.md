...
* Correct masked aggregation statistics and pseudobulk calculations from adata.X {pr}`720` {smaller}`S Dicks`
* {func}`~rapids_singlecell.tl.leiden` and {func}`~rapids_singlecell.tl.louvain` with ``use_dask=True`` now always tear down the cugraph communicator, even when the run fails, so a single failure no longer makes every later dask clustering call raise ``Communicator is already initialized`` {pr}`721` {smaller}`S Dicks`

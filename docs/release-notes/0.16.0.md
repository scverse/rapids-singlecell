...
```{rubric} Bug fixes
```
* {func}`~rapids_singlecell.pp.neighbors` with ``method="jaccard"`` no longer crashes with ``CUDA_ERROR_ILLEGAL_ADDRESS`` on large datasets. The shared-neighbor counts are now computed by a dedicated nanobind CUDA kernel directly from the KNN lists, instead of materializing a replicated adjacency intermediate whose ``nnz`` overflowed int32 once ``n_obs * (n_neighbors - 1)**2`` exceeded ``2**31`` {pr}`700` {smaller}`S Dicks`
